<script lang="ts">
    ...
    async function sellTicket(tokenId, ticketToSell) { 
        const { ethereum } = window as any;
        const provider = new ethers.BrowserProvider(ethereum);
        const signer = await provider.getSigner();
        const contract = await initializeContract(signer);
        ...
        const totalPrice = BigInt(ticket.ticketPrice) * BigInt(ticketToSell);
    ...
    function confirmPurchase() {
        sellTicket(selectedTicket.tokenId, ticketToSell);
        closeDialog();
    }
</script>
